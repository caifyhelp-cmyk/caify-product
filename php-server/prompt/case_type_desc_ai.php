...
function decode_json_int_list_desc(?string $json): array
{
...
function join_labels_desc(array $values, array $map): string
{
    $out = [];
    foreach ($values as $v) {
        if (isset($map[(int)$v])) {
...
$strengthsArr = decode_json_int_list_desc($promptRow['product_strengths'] ?? null);
...
$data = [
    ...
    'strengths' => join_labels_desc($strengthsArr, $mapStrengths),
];

$projectRoot = realpath(__DIR__ . '/..');
$activate    = $projectRoot . '/env/bin/activate';
$pyScript    = __DIR__ . '/case_type_desc_ai.py';
...
if ($output === null) {
    echo json_encode(['error' => 'Python 실행 실패'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (isset($response['response'])) {
    ...
    echo json_encode(['error' => 'AI 응답 오류', 'raw' => $output], JSON_UNESCAPED_UNICODE);
}
exit;
