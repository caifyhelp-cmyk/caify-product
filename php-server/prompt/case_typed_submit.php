...
$aiImageLayoutRaw = cs('ai_image_layout_json');
$targetKeywords = cs('target_keywords');
$caseInputType = cs('case_input_type');
$typedFormRaw = cs('typed_form_json');
$typedForm = [];
if ($typedFormRaw !== '') {
    $decoded = json_decode($typedFormRaw, true);
    if (is_array($decoded)) {
        $typedForm = $decoded;
    }
}
...
function buildTypedBackUrl(int $caseId, string $caseInputType): string
{
    $base = 'case_short_typed.php';
    $params = [];
    if ($caseId > 0) $params['id'] = $caseId;
    if ($caseInputType !== '') $params['type'] = $caseInputType;
    return $params ? $base . '?' . http_build_query($params) : 'case_type_select.php';
}
...
if (count($errors) > 0) {
    $backUrl = buildTypedBackUrl($editCaseId, $caseInputType);
...
if ($caseInputType !== '') {
    $aiStructured['input_case_type'] = $caseInputType;
}
...
$titleCandidatesRaw = cs('title_candidates_json');
...
if (count($typedForm) > 0) {
    $q1 = trim((string)($typedForm['q1'] ?? ''));
    $q2 = trim((string)($typedForm['q2'] ?? ''));
    $q3 = trim((string)($typedForm['q3'] ?? ''));
    if ($q1 !== '' || $q2 !== '' || $q3 !== '') {
        $aiStructured['typed_form'] = $typedForm;
    }
}
...
if (!is_dir($userUploadAbs)) {
    if (!mkdir($userUploadAbs, 0775, true) && !is_dir($userUploadAbs)) {
        case_swal_redirect('오류', '업로드 폴더 생성에 실패했습니다.', 'error', buildTypedBackUrl($editCaseId, $caseInputType));
    }
}
...
try {
    ...
    try {
        ...
        // best-effort: 분석 실패해도 저장은 유지
    }

    $successUrl = buildTypedBackUrl($caseId, $caseInputType);
    case_swal_redirect('저장 완료!', '고객 사례가 저장되었습니다.', 'success', $successUrl);
