...
function decode_json_int_list(?string $json): array
{
...
function join_prompt_labels(array $values, array $map): string
{
    $out = [];
...
if ($memberPk > 0) {
    ...
    if (is_array($promptRow)) {
        ...
        $strengths = join_prompt_labels(decode_json_int_list($promptRow['product_strengths'] ?? null), $mapStrengths);
        $tones = join_prompt_labels(decode_json_int_list($promptRow['tones'] ?? null), $mapTones);
        $contentStyles = join_prompt_labels(decode_json_int_list($promptRow['content_styles'] ?? null), $mapContentStyles);
        $expressions = join_prompt_labels(decode_json_int_list($promptRow['expressions'] ?? null), $mapExpression);
...
$pyScript    = __DIR__ . '/case_short_ai.py';
