...
function build_chat_raw_content_from_flow(array $typeCfg, array $chatFlow): string
{
    ...
    $q3Questions = is_array($chatFlow['question3']['questions'] ?? null) ? $chatFlow['question3']['questions'] : [];
    $q3Answer = trim((string)($chatFlow['question3']['answer'] ?? ''));
    $q3Skipped = !empty($chatFlow['question3']['skipped']);
    ...
    if (!$q3Skipped && (count($q3Questions) > 0 || $q3Answer !== '')) {
        $block = "[질문3]";
        if (count($q3Questions) > 0) {
            $lines = [];
            foreach ($q3Questions as $idx => $question) {
                $question = trim((string)$question);
                if ($question !== '') {
                    $lines[] = ($idx + 1) . '. ' . $question;
                }
...
$question3Questions = is_array($chatFlow['question3']['questions'] ?? null) ? array_values($chatFlow['question3']['questions']) : [];
$question3Answer = trim((string)($chatFlow['question3']['answer'] ?? ''));
$question3Skipped = !empty($chatFlow['question3']['skipped']);
...
$showFinalActions = $hasAiResult || $question3Skipped || count($question3Questions) > 0 || $question3Answer !== '';
                ...
                <div class="content content--chat-case">
                    ...
                    <div class="chat-progress-bar" id="chatProgressBar">
                        <div class="chat-progress-bar__steps">
                            ...
                            <div class="chat-progress-step__line"></div>
                            <div class="chat-progress-step" data-step="question3">
                                <span class="chat-progress-step__num">3</span>
                                <span class="chat-progress-step__label">질문 3</span>
                            </div>
                            <div class="chat-progress-step__line"></div>
                            ...
                            <div class="chat-progress-step" data-step="save">
                                <span class="chat-progress-step__num">6</span>
                    ...
                    <form id="chatCaseForm" enctype="multipart/form-data">
                            ...
                            <div class="chat-card__body">
                                ...
                                <div id="examplePanel" class="example-panel" style="display:none;">
                                    <div class="example-panel__head">
                                        ...
                                        <span>내 프롬프트 기준으로 생성된 예시입니다.</span>
                                    </div>
                                ...
                                <div class="chat-upload-inline">
                                    <div class="chat-upload-inline__head">
                                        <div>
                                            ...
                                            <p>여기 올린 이미지는 저장 시 AI 메타 분석을 수행하고, 이후 본문 이미지 자동 배치 추천에 활용됩니다.</p>
                                        </div>
                        ...
                        <section id="question2Card" class="chat-card" style="<?= (count($question2Questions) > 0 || $question2Answer !== '') ? '' : 'display:none;' ?> margin-top:10px;">
                            ...
                            <div class="chat-card__body">
                                ...
                                <div class="chat-actions">
                                    <button type="button" id="btnQuestion2Complete" class="btn btn--primary">답변 완료하고 AI 분석 받기</button>
                                </div>
                            </div>
                        </section>

                        <section id="question3Card" class="chat-card" style="<?= ((count($question3Questions) > 0 && !$question3Skipped) || $question3Answer !== '') ? '' : 'display:none;' ?> margin-top:10px;">
                            <div class="chat-card__head">
                                <div>
                                    <span class="chat-card__step">질문 3</span>
                                    <h3 class="chat-card__title">거의 다 왔습니다! 아래 추가 질문에 최대한 구체적으로 답변해주세요.</h3>
                                </div>
                            </div>
                            <div class="chat-card__body">
                                <div id="question3List" class="chat-followup-list"></div>
                                <textarea id="question3Answer" name="question3_answer" class="chat-textarea" rows="8" placeholder="질문 3 답변을 입력해주세요."><?= htmlspecialchars($question3Answer, ENT_QUOTES, 'UTF-8') ?></textarea>
                            </div>
                        </section>

                        <div id="question3SkipNote" class="chat-skip-note" style="<?= $question3Skipped ? '' : 'display:none;' ?>">질문 2 답변으로 충분하여 질문 3은 생략되었습니다.</div>
...
.draft-image-slot { padding: 32px; border: 1px solid #e4e4e7; border-radius: 24px; background: #ffffff; transition: box-shadow 0.3s; }
...
(function () {
    ...
    function buildRawContent() {
        ...
        var q3 = (document.getElementById('question3Answer').value || '').trim();
        var q3Skipped = document.getElementById('question3Skipped').value === '1';
        var blocks = [];
        ...
        if (!q3Skipped && (question3Questions.length || q3)) {
            var lines3 = ['[질문3]'];
            question3Questions.forEach(function (question, index) {
                lines3.push((index + 1) + '. ' + question);
            });
            if (q3) {
                lines3.push('', '답변:', q3);
            }
            blocks.push(lines3.join('\n'));
        }
    ...
    function callChatAi(payload) {
        return fetch('case_typed_chat_ai.php', {
            method: 'POST',
    ...
    function updateProgressBar() {
        var steps = [
            ...
            { key: 'question3', check: function () {
                var skipped = document.getElementById('question3Skipped').value === '1';
                if (skipped) return (document.getElementById('question2Answer').value || '').trim() !== '';
                if ((question3Questions || []).length === 0) return false;
                return (document.getElementById('question3Answer').value || '').trim() !== '';
            }},
    ...
    function updateQuestionStateUI() {
        ...
        document.getElementById('question3QuestionsJson').value = JSON.stringify(question3Questions || []);
        renderFollowupList('question2List', question2Questions);
        renderFollowupList('question3List', question3Questions);

        var q2Card = document.getElementById('question2Card');
        var q3Card = document.getElementById('question3Card');
        var q3SkipNote = document.getElementById('question3SkipNote');
        ...
        q3Card.style.display = (!q3Skipped && (question3Questions.length || document.getElementById('question3Answer').value.trim())) ? '' : 'none';
        q3SkipNote.style.display = q3Skipped ? '' : 'none';
        if (finalActions) {
            finalActions.style.display = (!aiResultVisible && (q3Skipped || question3Questions.length > 0)) ? '' : 'none';
        }
        updateProgressBar();
    }
    ...
    function validateBeforeGenerate() {
        ...
        if (question3Questions.length > 0 && !q3Skipped && !q3) return '질문 3 답변을 입력하거나 생략 상태여야 합니다.';
        return '';
    }
    ...
    function renderExample(index) {
        ...
        var combined = '1. ' + typeQuestionLabels[0] + '\n' + ex.q1 + '\n\n2. ' + typeQuestionLabels[1] + '\n' + ex.q2 + '\n\n3. ' + typeQuestionLabels[2] + '\n' + ex.q3;
    ...
    function runGenerateFlow() {
        ...
        saveCaseOnly({ silentMessage: true })
            .then(function () {
                buildRawContent();
                return callChatAi({
                    mode: 'analyze',
    ...
    document.getElementById('btnQuestion1Complete').addEventListener('click', function () {
        ...
        callChatAi({
            ...
            question2Questions = json.data.questions.slice(0, 3);
            document.getElementById('question3Skipped').value = '0';
            question3Questions = [];
            document.getElementById('question2Card').style.display = '';
            document.getElementById('question3Card').style.display = 'none';
            document.getElementById('question3SkipNote').style.display = 'none';
            updateQuestionStateUI();
    ...
    document.getElementById('btnQuestion2Complete').addEventListener('click', function () {
        var q1 = (document.getElementById('question1Answer').value || '').trim();
        var q2 = (document.getElementById('question2Answer').value || '').trim();
        if (!q2) {
            alert('질문 2 답변을 먼저 입력해주세요.');
            return;
        }
        showLoading('추가 질문 여부를 확인하고 있습니다', '질문 3이 필요한지 AI가 판단하고 있습니다.');
        callChatAi({
            mode: 'followup3',
            case_input_type: caseInputType,
            question1_answer: q1,
            ...
            hideLoading();
            if (!json.success || !json.data) {
                throw new Error(json.error || '질문 3 판단에 실패했습니다.');
            }
            if (json.data.need_more && Array.isArray(json.data.questions) && json.data.questions.length) {
                question3Questions = json.data.questions.slice(0, 3);
                document.getElementById('question3Skipped').value = '0';
                document.getElementById('question3Card').style.display = '';
                document.getElementById('question3SkipNote').style.display = 'none';
                updateQuestionStateUI();
                document.getElementById('question3Card').scrollIntoView({ behavior: 'smooth', block: 'start' });
            } else {
                question3Questions = [];
                document.getElementById('question3Skipped').value = '1';
                document.getElementById('question3Card').style.display = 'none';
                document.getElementById('question3SkipNote').style.display = '';
                updateQuestionStateUI();
            }
        }).catch(function (error) {
            hideLoading();
            alert(error && error.message ? error.message : '질문 3 판단에 실패했습니다.');
        });
    });
    ...
    ['question1Answer', 'question2Answer', 'question3Answer'].forEach(function (id) {
