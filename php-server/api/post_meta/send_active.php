...
try {
    ...
    $stmt = $pdo->query(
        'SELECT * FROM caify_prompt
         WHERE is_active = 1
           AND (timer IS NULL OR DATE(timer) <> CURDATE())
         ORDER BY id ASC'
    );
    $rows = $stmt->fetchAll();
    ...
    $fileStmt = $pdo->prepare('SELECT * FROM caify_prompt_file WHERE prompt_id = :id ORDER BY id DESC');
    $updateStmt = $pdo->prepare('UPDATE caify_prompt SET timer = CURDATE() WHERE id = :id');
    ...
    echo json_encode(['ok' => false, 'error' => '서버 오류가 발생했습니다.'], JSON_UNESCAPED_UNICODE);
}
