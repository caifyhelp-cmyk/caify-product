      ...
      (function() {
        ...
        if (iDoc2) {
          ...
          s2.textContent = '(function(){'
            ...
            + '        var compTypes=comps.map(function(c){return (c.ctype||c.type||"?")+":"+Object.keys(c).join(";");}).join("|");'
            + '        var textComp=null;'
            + '        for(var ci=0;ci<comps.length;ci++){'
            + '          var ct=(comps[ci].ctype||comps[ci].type||"").toLowerCase();'
            + '          if(ct&&ct!=="documenttitle"&&ct!=="title"&&ct!=="document_title"){'
            + '            textComp=comps[ci];break;'
            + '          }'
            + '        }'
            + '        if(textComp){'
            + '          var bodyKey=("html" in textComp)?"html":("content" in textComp)?"content":("data" in textComp)?"data":null;'
            + '          if(!bodyKey){document.__caifyResult2="no_bodyKey:compKeys=["+Object.keys(textComp).join(",")+"],types="+compTypes;return;}'
            + '          textComp[bodyKey]=h;'
            + '          ds2.setDocumentData(docData);'
            + '          document.__caifyResult2="ok_setDocData:"+bodyKey+":"+ct;return;'
            + '        }'
