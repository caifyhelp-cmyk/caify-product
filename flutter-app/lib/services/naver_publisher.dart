      ...
      (function() {
        ...
        if (iDoc2) {
          ...
          s2.textContent = '(function(){'
            ...
            + '        }'
            + '        if(textComp){'
            // value 구조 파악용 진단 — 수정 전 현재 값을 로깅
            + '          var valPreview=JSON.stringify(textComp.value||textComp.html||textComp.content||"?").substring(0,200);'
            + '          var compKeys=Object.keys(textComp).join(",");'
            + '          document.__caifyResult2="textComp_val:keys=["+compKeys+"],val="+valPreview;return;'
            + '        }'
